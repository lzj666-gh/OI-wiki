...
线段树在信息合并时间复杂度为 $O(D)$ 的情况下可以在 $O(D\log N)$ 的时间复杂度内实现单点修改、区间修改、区间查询（区间求和，求区间最大值，求区间最小值）等操作。

## 线段树的基本结构与建树

### 过程

线段树将每个长度不为 $1$ 的区间划分成左右两个区间递归求解，把整个线段划分为一个树形结构，通过合并左右两区间信息来求得该区间的信息。这种数据结构可以方便的进行大部分的区间操作。我们在这里以区间加法为例。

有个大小为 $5$ 的数组 $a=\{10,11,12,13,14\}$，要将其转化为线段树，我们可以采用以下这种做法：设线段树的根节点编号为 $1$，用数组 $d$ 来保存我们的线段树，$d_i$ 用来保存线段树上编号为 $i$ 的节点的值（这里每个节点所维护的值就是这个节点所表示的区间总和）。
...
我们在这种建树方式下规定，$d_i$ 的左儿子节点就是 $d_{2\times i}$，$d_i$ 的右儿子节点就是 $d_{2\times i+1}$。如果 $d_i$ 表示的是区间 $[s,t]$（即 $d_i=a_s+a_{s+1}+ \cdots +a_t$）的话，那么 $d_i$ 的左儿子节点表示的是区间 $[ s, \frac{s+t}{2} ]$，$d_i$ 的右儿子表示的是区间 $[ \frac{s+t}{2} +1,t ]$。

当然，你可以使用其他的建树方式，但是请注意，线段树的左右两个儿子区间的大小差距应当尽量小，否则没有时间复杂度保证。
...
这样的线段树可以自底向上维护，参考「[统计的力量 - 张昆玮](https://github.com/hzwer/shareOI/blob/master/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/%E7%BB%9F%E8%AE%A1%E7%9A%84%E5%8A%9B%E9%87%8F%E2%80%94%E2%80%94%E7%BA%BF%E6%AE%B5%E6%A0%91%E5%85%A8%E6%8E%A5%E8%A7%A6_%E5%BC%A0%E6%98%86%E7%8E%AE.pptx)」，这种线段树也称为**zkw 线段树**。
...
同理，你可以自行推导区间乘法版本，区间 $\gcd$ 版本（注意：这个版本的区间信息合并是 $O(\log V)$ 级别的，所以实际上这种线段树它的修改和查询是 $O(\log n\log V)$ 级别的）。

## 线段树的区间修改与懒惰标记

### 过程

如果要求修改区间 $[l,r]$，把所有包含在区间 $[l,r]$ 中的节点都遍历一次、修改一次，时间复杂度无法承受。我们这里要引入一个叫做 **「懒惰标记」** 的东西（也可简称为懒标记）。

懒惰标记，简单来说，就是通过延迟对节点信息的更改，从而减少可能不必要的操作次数。每次执行修改时，我们通过打标记的方法表明该节点对应的区间在某一次操作中被更改，但不更新该节点的子节点的信息。实质性的修改则在下一次访问带有标记的节点时才进行。

注意：懒惰标记要求你知道一个修改产生的影响对于一个区间的答案可以快速计算时才适用。例如：区间加法以后我们不能快速统计区间按位异或的答案，所以不能使用懒惰标记。
...
接下来给出在存在标记的情况下，区间修改和查询操作的参考实现（仍以区间加法，区间求和为例）。
...
如果你是要实现区间修改为某一个值而不是加上某一个值的话（这种情况仍可快速统计区间答案），代码如下：
